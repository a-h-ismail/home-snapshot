...
exclude_relative=`realpath --relative-to="$source_dir" "$destination_dir"`

if [ -z "$(echo "$exclude_relative" | grep "^\.\./")" ]; then
    exclude_relative=`echo "--exclude='$exclude_relative'"`
else
    unset exclude_relative
fi
...
if [ -z "$(echo * | grep "$today_date")" ]; then
    mkdir -p "$backup_path"
    rsync -aXh --stats --delete "$source_dir/" $no_perms --link-dest "$latest_link" \
    --exclude-from="$HOME/.config/home-snapshot-excl.conf" $exclude_relative "$backup_path" 2> $log_location
